...
public class Main {
    ...
    */
    ...
    //Daha önceden kaydedilmiş bir oyunumuz var mı kontrol ediyoruz
    File file = new File("kayit.dat");// Oluşturduğum nesne dosyayı işaret ediyor sadece önceden varolduğu için
    
    @SuppressWarnings("resource")
	Scanner scanner = new Scanner(System.in);
    // Yani böyle bir dosya var mı
    if (file.exists()) { // Exists metodu belirtilen dosyanın daha önceden oluşturulup oluşturulmadığını kontrol eder dosya varsa true
    					// dosya yoksa false değer döner
    	System.out.print("Kaydedilmiş bir oyununuz var. Kayıttan devam etmek ister misiniz ?(yes ya da no): ");
		String cevap = scanner.nextLine();
		if (cevap.equals("yes")) {
			kartlar = OyunKayit.kayitAc();
			return; // void olmasına rağmen return dediğimizde direkt olarak metod sonlanıyor
		}
	}
    kartlar[0][0] = new Kart('E');
    kartlar[0][1] = new Kart('A');
